...
namespace Catan.Core.Engine
{
    public class HexMap
    {
        ...
        public HexMap(IRandomProvider random, int radius = 2)
        {
            Radius = radius;
            _random = random;
        }
        ...
        public void GetPortsEdges()
        {
            ...
            var indices = Enumerable.Range(0, PortNumber).ToList();
            indices.Shuffle(_random);
            indices = indices.Take(remainder).ToList();
